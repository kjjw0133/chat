...
begin
  ...
  begin
    // Panel 생성
    panel := TPanel.Create(ScrollBox1);   // Owner로 ScrollBox1을 줘서 자동 해제
    panel.Parent := ScrollBox1;
    panel.Align := alTop;
    panel.Height := 60;
    panel.Caption := '';
    panel.Top := VerticalOffset;
    panel.Tag := FDQuery1.FieldByName('recommended_userno').AsInteger; // 클릭 시 사용할 ID
    panel.OnClick := PanelRecommendClick; // 이벤트 연결
  ...
  // 필요 시 FDQuery1.Close; // 또는 유지
end;

// 클릭 이벤트 예시 (폼에 아래 메서드를 추가)
...
begin
  ...
  begin
    pnl := TPanel(Sender);
    recommendedUserNo := pnl.Tag; // 우리가 저장해둔 UserNo
    // 여기에 친구추가 로직 작성 (예: 친구 요청 다이얼로그, DB insert 등)
