...
3. 실행하기 전 Project -> options -> Application -> Froms -> Auto-create forms(Form1,2,3,4,5,6,7,8), Availabel forms(Form9,10,11,13,14)을 넣고 저장
...
11. 채팅 방 생성 시 친구 추가된 유저를 선택해서 방을 생성할 수 있음, 방을 생성한 후 채팅방을 보여주지 않고 메인을 보여줌, 내가 만든 방에 바로 입장하도록 변경

12. 메인에서 사이드 메뉴로 친구 창으로 이동, 해당 페이지에서 친구를 확인 할 수 있음, 유저 id를 이용한 친구 추가 기능 구현
--- 
...
+ 같은 채팅방에 있는 유저 중 친구 추가가 안된 유저를 추천 친구로 지정한다.
+ 앞에 친구 추가 기능을 구현헀으면 다음으로 친구 추가 수락, 거절 기능을 만들어야 함, 메인 페이지 사이드 메뉴에 있는 더보기에 수락,거절 기능 구현, ScrollBox안에 보낸 유저 이름, 수락,거절 버튼을 보여주고 각 버튼을 클릭하면 friend테이블의 status 컬럼을 수락 : 2, 거절 : 0
