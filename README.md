...
2. form7에서 친구를 볼 수 있도록 사이드 메뉴(바)를 만들고 거기에 친구, 메인 채팅 창(기존 메인) , 더보기 등 form을 만들어 기능을 추가하고 같은 ui을 사용하여 사이드 바에 메뉴를 누를때마다 form을 변경시켜줌
...
```
   ...
   form14.Close;
```
+ 이런식으로 코드를 작성했는데, 7번은 화면에 나오지 않고 프로그램이 사라져
