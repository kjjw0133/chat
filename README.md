...
문제 5: 쿼리문 다르게 출력
증상 : DB에서 실행한 쿼리는 가장 최근에 채팅이 올라온 채팅방이 위에 위치하도록 쿼리 작성, 델파이에서 값을 넣으면 DESC로 정렬한 값이 반대인 ASC로 출력되는 상황 
원인 : Panel을 생성하여 그 안에 Label에 쿼리 값을 넣는데 Panel 생성 순서가 위에서 생성이 되고 두번째 생성되면 첫번째 값이 아래로 가는 방식으로 생성, 그래서 쿼리에 첫번째 값은 가장 최근에 메시지가 입력된 채팅방을 출력하니 델파이에서 Panel의 생성과 만나면서 반대로 출력되는 현상 발생
해결 방안 : 
+ Panel 생성 위치 변경
+ 쿼리문 DESC에서 ASC로 변경
