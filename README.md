...
2. 가끔가다 로그아웃을 해도 메인 폼에 있는 Panel이 사라지지 않는 오류가 있음, 로그아웃이 정상적으로 되지 않아서 채팅도 들어가지는 것으로 확인된다. (그래서 수정을 하면 채팅방에 들어갈때 로그인 상태를 확인, 아이디와 이름 등의 값이 정상적인지 확인 후 채팅방 입장,그리고 로그아웃이 정상적으로 안되는 원인을 찾아서 수정할 예정)
